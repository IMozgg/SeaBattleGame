...
public class GameModel implements IGame {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = i; j < 4; j++) {
                ...
                do {
                    ...
                    } while (checkFreeSpaceAboutShip(newPoint, i + 1, randomAttitude, player) > (i + 3));
    ...
    private Point finishOffShip(Player player) {
        Point tempPoint = new Point();
        int tempX = 0;
        int tempY = 0;
        ...
        while (!shootAccess) {
            ...
            if ((tempY >= 0 & tempY < player.getSea().getSize()) & (tempX >= 0 & tempX < player.getSea().getSize())) {
                if ((player.getMaskShipSea()[tempX][tempY] == 0) & (player.getSeaShoot()[tempX][tempY] == -2)) {
    ...
    public Point iiShoot(Player player) {
        ...
        if (!isContinueShoot) {
            if (isRandomShoot) {
                ...
                } while (player.getMaskShipSea()[tempX][tempY] != 0 | player.getSeaShoot()[tempX][tempY] != -2);
                ...
                do {
                    ...
                    if ((pointShoot.getX() >= 0 & pointShoot.getX() < sizeSea) & (pointShoot.getY() >= 0 & pointShoot.getY() < sizeSea)) {
                        //  Проверяем, что игрок туда не стрелял и нет маски
                        if (player.getSeaShoot()[pointShoot.getX()][pointShoot.getY()] == -2 & player.getMaskShipSea()[pointShoot.getX()][pointShoot.getY()] == 0) {
                            //  Если все выполнено, то повторять не надо
                            isRepeat = false;
                        } else {
                            //  Если уже стреляли или маска, то повторим блок кода
