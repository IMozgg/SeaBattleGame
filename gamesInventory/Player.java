...
public class Player {
    ...
    public int shoot(Point p) {
        if (seaShoot[p.getX()][p.getY()] == 0) {
            seaShoot[p.getX()][p.getY()] = 1;
            for (Ship ship : ships) {
    ...
    public void applyMaskPoint(Point p) {
        maskShipSea[p.getX()][p.getY()] = 1;
    }
    ...
    public void testPrintShip() {
        for (Ship curShip : ships) {
            ...
            for (int i = 0; i < curShip.getSize(); i++) {
                System.out.print(" (" + (i + 1) + ") - " + curShip.getShipOnCells()[i]);
            }
            System.out.println("\tSIZE\t" + curShip.getSize());
        }
    }
